...
void Game::UpdateEntities() noexcept
{
	...
	const float offset = player.GetX() * -1.0f;
...
void Game::HandleCollisions() noexcept
{
	...
	for (auto& projectile : playerProjectiles) {
		...
		for (auto& alien : Aliens) {
			...
			if (CheckCollisionRecs(projectile.Hitbox(), alien.Hitbox())) {
	...
	for (auto& projectile : enemyProjectiles) {
		if (!projectile.active) continue;
		if (CheckCollisionRecs(projectile.Hitbox(), player.Hitbox())) {
			projectile.active = false;
			player.TakeDamage();
		}
	}

	// Player projectiles vs walls
	for (auto& projectile : playerProjectiles) {
		if (!projectile.active) continue;
		for (auto& wall : Walls) {
			if (!wall.IsActive()) continue;
			if (CheckCollisionRecs(projectile.Hitbox(), wall.Hitbox())) {
				projectile.active = false;
				wall.Hit();
				break;
			}
...
void Game::HandleAlienShooting()
{
	...
	spawnPos.y += ALIEN_SHOOT_Y_OFFSET;

	enemyProjectiles.emplace_back(spawnPos, -Projectile::DEFAULT_SPEED);
}

void Game::RemoveInactiveEntities()
{
	std::erase_if(playerProjectiles, [](const Projectile& p) {
		return !p.active;
		});

	std::erase_if(enemyProjectiles, [](const Projectile& p) {
		return !p.active;
		});
...
void Player::Update() noexcept
{
	...
	if (x_pos < RENDER_SIZE / 2.0f)
	{
		x_pos = RENDER_SIZE / 2.0f;
	}
	else if (x_pos > GetScreenWidth() - RENDER_SIZE / 2.0f)
	{
		x_pos = GetScreenWidth() - RENDER_SIZE / 2.0f;
	}
...
void Player::Render(const Texture2D& texture) const noexcept
{
	...
	DrawTexturePro(texture,
		...
		},
		{
			...
			RENDER_SIZE,
			RENDER_SIZE,
		}, { RENDER_SIZE / 2.0f, RENDER_SIZE / 2.0f },
		0,
		WHITE);
}
...
void Projectile::Update() noexcept
{
	...
	if (position.y < -RENDER_SIZE / 2.0f || position.y > static_cast<float>(GetScreenHeight()) + RENDER_SIZE / 2.0f)
	{
		active = false;
	}
}
